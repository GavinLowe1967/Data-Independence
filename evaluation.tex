...
for example, the expression \CSPM{\{\|c2\|\}}  defines the same set as
\CSPM{\{c2.x.y \| x <- t, y <- t\}}.  By the assumption that $\rho$ is
...
%% \begin{conjecture}
%% Let $\rho(t) = T$.  \framebox{Probably need an assumption about $\rho$}
%% %
%% \begin{enumerate}
%% \item If $e$ is an expression defining a set, and $e$ makes no use of~$t$,
%%   then $\eval \rho~e$ is $T$-invariant. 

%% \item If $d$ is the name of a datatype, then $\eval \rho~d$ is $T$-invariant.

%% \item If $C$ is a constructor of a datatype, then $\eval \rho~\eset{C}$ is
%%   $T$-invariant. 

%% \item If $s$ is a set comprehension $\set{

%% .........

%% \item If $S_1$ and $S_2$ are $T$-invariant, then so is $S_1 \union S_2$.
%% \end{enumerate}
%% \end{conjecture}
...
\framebox{Show} a set is $T$-invariant if it doesn't depend on $t$; is of the
...
Clearly $\rho^\bottom$ is $T$-invariant.  Lemma~\ref{lem:invariant} shows that
